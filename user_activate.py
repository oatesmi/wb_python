...
def call(url):
    ...
    for row in data:
        ...
        headers = {'Authorization': bearer,'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; rv:91.0) Gecko/20100101 Firefox/91.0'}
        json={
            "user_id":row['user_id'],
            "email":row["email"],
            #"time_zone":row["time_zone"],
            #"profile":{"company":"HDI Global Insurance"},
            "status":"active"
        }
        try:
            response = requests.put(url + str(row['user_id']), headers=headers, json=json, timeout=15)
        except Timeout:
